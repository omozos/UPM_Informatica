...
Los vectores se pasan a las funciones siempre por referencia. 
En la definicion y declaraqcion de la funcion se usa el nombre de la variable y los corchetes vacios []. 
Dentro de la funcion se usa de forma habitual.

** Ejemplo (1)  vector de enteros

#include "stdio.h" 

void mostrar( int n[], int s ) {
	int i;
	for (i=0; i<s; i++) {   
		printf("%d ", n[i]); 
	}
...
int main (void) {
	int combinacion[3]={1,56,78}; 

    mostrar( combinacion, 3 ); 
	return 0;
}
...
** Ejemplo (3) acceso a elementos de cadena de texto
...
** Ejemplo (4) vector de estructuras

#+BEGIN_SRC C
#include "stdio.h" 

// N sera sustituido por 2
#define N 2   
...
Asignatura curso[N]={{"matematicas",7}, {"ingles",7.5}};

// función 
void mostrar( Asignatura lista[] ) {
	int i;
	for (i=0; i<N; i++) {   
		printf("\nAsignatura: %s\nCreditos: %.2f \n", lista[i].nombre, lista[i].creditos); 
	}
}
